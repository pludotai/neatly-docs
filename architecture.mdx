...
```mermaid
flowchart LR
  operator[Operations user] --> web[Web application\nTanStack Start + Nitro]
  web --> db[(PostgreSQL)]
  web --> skyvern[Skyvern API]
  web --> files[S3-compatible storage]
  web --> timer[Nitro scheduled task\nevery minute]
  timer --> db
  timer --> skyvern
  mailbox[OTP mailbox\nMicrosoft Graph] --> email[Email service\nGo HTTP API + watcher]
  email --> graph[Microsoft Graph]
  email --> skyvern
  web --> otp[Web OTP ingress]
  otp --> skyvern
```
